...
int main()
{
  ...
  std::cout << "before encrypt " << plainText << std::endl;
  aES256EncryptString(plainText, encryptedText, key, iv);
  //std::cout << "encryptedText: " << encryptedText << std::endl;
  aES256DecryptString(encryptedText, decryptedText, key, iv);
  std::cout << "decryptedText: " << decryptedText << std::endl;



  const int multipal = 4;
  byte toEncrypt[] = { 5, 9, 0, 4, 3, 100, 128, 0, 99, 5, 9, 0, 4, 3, 100, 128, 0, 99 };
  int bufferLen = sizeof(toEncrypt) * multipal;
  std::unique_ptr<byte[]> cipherText = std::make_unique<byte[]>(bufferLen);
  std::unique_ptr<byte[]> recoveredText = std::make_unique<byte[]>(bufferLen);
  int lenInOutEn = bufferLen;
  std::cout << "before encrypt "  << std::endl;
  for (int i = 0; i < sizeof(toEncrypt); i++) {
    std::cout << std::hex << (int)toEncrypt[i] << " ";
  }

  aES256Encrypt(toEncrypt, sizeof(toEncrypt),  cipherText.get(), lenInOutEn, key, iv);
  std::cout << std::endl << "Encrypted: count "  << lenInOutEn << std::endl;

 // for (int i = 0; i < lenInOutEn; i++) {
  //  std::cout << std::hex << (int)cipherText[i] << " ";
 // }
  int lenInOutDe = bufferLen;
  aES256Decrypt(cipherText.get(), lenInOutEn, recoveredText.get(), lenInOutDe, key, iv);
